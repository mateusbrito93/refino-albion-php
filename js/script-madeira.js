...
async function calcular() {
    ...
    try {
        ...
        const barraAnteriorData = barraAnteriorItem ? dadosTabuaAnt?.find(d => d.item_id === barraAnteriorItem) : null;
        const barraAtualData = dadosTabuaAtual.find(d => d.item_id === barraAtualItem);
        ...
        if (tier > 2 && !barraAnteriorData?.sell_price_min) {
            errorMessage += `<div class="flex items-center gap-3 mt-2"><img src="img/T${tier - 1}_PLANKS${encSuffixTabuaAntError}.png" alt="Tábua Anterior" class="h-8 w-8"><span>Preço não encontrado para <strong>${barraAnteriorItem}</strong> em ${cidadeCompra}</span></div>`;
        }
        if (!barraAtualData?.sell_price_min) {
            errorMessage += `<div class="flex items-center gap-3 mt-2"><img src="img/T${tier}_PLANKS${encSuffixTabuaAtualError}.png" alt="Tábua Atual" class="h-8 w-8"><span>Preço não encontrado para <strong>${barraAtualItem}</strong> em ${cidadeVenda}</span></div>`;
        }
        ...
        const precoTabua = barraAnteriorData?.sell_price_min || 0;
        ...
        const totalTabuaAnterior = barrasAnteriores[tier] * quantidade;
                ...
                <div class="bg-gray-700 p-4 rounded-lg shadow">
                    ...
                    <div class="space-y-2">${imagensMateriais}
                        <div class="pt-2 mt-2 border-t border-gray-600">
                            ...
                            <span>Tábuas Retornados: ${Math.floor(barrasRetornados)}</span>
                        </div>
                    </div>
                </div>
                <div class="bg-gray-700 p-4 rounded-lg shadow">
        ...
        document.getElementById("resultado").innerHTML = `<div class="bg-red-900 text-white p-4 rounded-lg"><p class="font-bold"><i class="fas fa-exclamation-triangle mr-2"></i>Erro ao calcular</p><p class="mt-2">${err.message}</p><p class="mt-2 text-sm">Itens buscados:</p><ul class="text-sm mt-1"><li>${madeiraItem}</li>${barraAnteriorItem ? `<li>${barraAnteriorItem}</li>` : ''}<li>${barraAtualItem}</li></ul><button onclick="calcular()" class="mt-4 bg-red-700 hover:bg-red-600 text-white px-4 py-2 rounded-lg"><i class="fas fa-sync-alt mr-2"></i>Tentar novamente</button></div>`;
        finalizarBarraProgresso();
        scrollParaResultados();
    }
}
...
async function calcularall() {
    ...
    for (let tier of tiersParaCalcular) {
        const madeiraItem = `T${tier}_WOOD${enc > 0 ? '_LEVEL' + enc + '@' + enc : ''}`;
        const barraAnteriorItem = tier > 2 ? `T${tier - 1}_PLANKS${(tier - 1 >= 4 && enc > 0) ? '_LEVEL' + enc + '@' + enc : ''}` : null;
        const barraAtualItem = `T${tier}_PLANKS${enc > 0 ? '_LEVEL' + enc + '@' + enc : ''}`;
        const urlMadeira = `https://west.albion-online-data.com/api/v2/stats/prices/${madeiraItem}.json?locations=${cidadeCompraFormatada}`;
        const urlTabuaAnterior = barraAnteriorItem ? `https://west.albion-online-data.com/api/v2/stats/prices/${barraAnteriorItem}.json?locations=${cidadeCompraFormatada}` : null;
        const urlTabuaAtual = `https://west.albion-online-data.com/api/v2/stats/prices/${barraAtualItem}.json?locations=${cidadeVendaFormatada}`;
        ...
        try {
            ...
            if (!madeiraData?.sell_price_min || !barraAtualData?.sell_price_min || (tier > 2 && !barraAnteriorData?.sell_price_min)) {
            ...
            const lucro = receita - custoTotal;
            const rentabilidade = custoTotal > 0 ? ((lucro / custoTotal) * 100).toFixed(2) : "0.00";
            resultadosHTML += `<tr><td class="px-4 py-2 font-bold">T${tier}.${enc}</td><td class="px-4 py-2">${totalMadeira} x $${formatarValor(precoMadeira)}</td><td class="px-4 py-2">${tier > 2 ? totalTabuaAnterior + ' x $' + formatarValor(precoTabua) : '-'}</td><td class="px-4 py-2">${quantidade} x $${formatarValor(precoVenda)}</td><td class="px-4 py-2">${totalMadeira + (tier > 2 ? totalTabuaAnterior : 0)}</td><td class="px-4 py-2">${Math.floor(barrasRetornados)}</td><td class="px-4 py-2">${producaoTotal.toFixed(2)}</td><td class="px-4 py-2">$${formatarValor(custoTotal)}</td><td class="px-4 py-2">$${formatarValor(receita)}</td><td class="px-4 py-2 ${lucro >= 0 ? 'text-green-500' : 'text-red-500'}">$${formatarValor(lucro)}</td><td class="px-4 py-2 ${rentabilidade >= 0 ? 'text-green-500' : 'text-red-500'}">${rentabilidade}%</td></tr>`;
        } catch (error) {
